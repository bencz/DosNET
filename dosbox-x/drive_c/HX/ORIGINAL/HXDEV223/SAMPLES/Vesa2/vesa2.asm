...
startlabel textequ <mainCRTStartup>

else

;--- this code assumes that start module InitPM - instead of JmpPM32 - is used
;--- that's less overhead, but requires a few tasks to be done before main is called
...
;--- set __baseadd to base of DS and set its limit to -1 (since dynamically allocated memory is used)
    mov ebx,ds
    mov ax,6
    int 31h
    mov word ptr [__baseadd+0],dx
    mov word ptr [__baseadd+2],cx
    mov cx,-1
    mov edx,ecx
    mov ax,8
    int 31h

;--- init GS as flat, zero-based selector
    mov ax,0
    mov cx,1
    int 31h
    mov gs,eax
